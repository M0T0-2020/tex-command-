...
$\displaystyle \\ y$が$y$の任意の近傍$I$に対して$k \in \mathbb{N}$が存在して$P(S_{k} \in I) >0$となるときyは候補状態であるとする.
...
$\\ x$は再帰的だから$\displaystyle P(\bigcap_{m \in \mathbb{N} } \bigcup_{n > m} \left\{ \left| S_{n} - x \right| < \varepsilon \right\}) = 1$となるから,
$\displaystyle  \\
...
$
$\displaystyle \because \displaystyle \omega \in \left\{ \left| S_{k} - y \right|  < \varepsilon \right\} \cap \bigcup_{m \in \mathbb{N} } \bigcap_{n > m} \left\{ \left| S_{k+n} - S_{k} - (x-y) \right| \ge 2\varepsilon \right\}$をとると,$\displaystyle {}^\exists m \in \mathbb{N} \ s.t. \\ {}^\forall n \ge m, \ \left| S_{k+n}(\omega) - S_{k}(\omega) -(x-y) \right| \ge 2 \varepsilon$となる.

...
ここで, $\displaystyle \left\{ \sup_{ m < n} \left| \sum_{ k = m+1 }^{n} X_{k} \right| > \varepsilon \right\}$は$m$に関して単調減少なので,

%考察が必要
$\\ \displaystyle \lim_{m \to \infty}  P\left( \sup_{ m < n} \left| \sum_{ k = m+1 }^{n} X_{k} \right| > \varepsilon \right) = P\left( \lim_{m \to \infty} \sup_{ m < n} \left| \sum_{ k = m+1 }^{n} X_{k} \right| > \varepsilon \right) = $ 


$\\ \\ \\ \\ $

$\displaystyle P\left( \varlimsup_{m \to \infty} \left| \sum_{ k = m+1 }^{n} X_{k} \right| > \varepsilon \right) \ge P\left( \lim_{m \to \infty} \left| \sum_{ k = m+1 }^{n} X_{k} \right| > \varepsilon \right)$となる.つまり $\displaystyle P\left( \lim_{m, n \to \infty} \left| \sum_{ k = m+1 }^{n} X_{k} \right| > \varepsilon \right) = 0$. よって$\displaystyle \sum_{k=1}^{n}X_{k}$がほとんど確実にCauchy列であることがわかった. これは$\displaystyle \sum_{k=1}^{n}X_{k}$が概収束しないことに矛盾する.
